...
function output = Qabf_eval(strA, strB, strF)
...
pA = (strA); pA = double(pA);
pB = (strB); pB = double(pB);
pF = (strF); pF = double(pF);


SAx = conv2(pA,h3,'same');
SAy = conv2(pA,h1,'same');
...
SBx = conv2(pB,h3,'same'); 
SBy = conv2(pB,h1,'same');
